 ...
 */
/**
 * Calculate deterministic (non-random) damage modifiers
 * Returns the pre-random product of baseDamage * all deterministic modifiers
 */
function calculateDeterministicDamage(
    attacker: Actor,
    defender: Actor,
    distance: number,
    ...
    config: CombatConfig
): { baseDamage: number; distanceMod: number; terrainMod: number; coverMod: number; armorMod: number; criticalMod: number; isCritical: boolean } | null {
    ...
    return { baseDamage, distanceMod, terrainMod, coverMod, armorMod, criticalMod, isCritical };
}
...
export function calculateDamage(
    ...
    const det = calculateDeterministicDamage(attacker, defender, distance, attackerTerrain, defenderTerrain, options, config);
    if (!det) {
        return createZeroDamageResult('Attacker has no weapon');
    }
    const { baseDamage, distanceMod, terrainMod, coverMod, armorMod, criticalMod, isCritical } = det;
...
export function calculateExpectedDamage(
    ...
    const det = calculateDeterministicDamage(attacker, defender, distance, attackerTerrain, defenderTerrain, {}, config);
    if (!det) return 0;

    const { baseDamage, distanceMod, terrainMod, coverMod, armorMod, criticalMod } = det;

    // Apply average random modifier to get expected damage (no lossy reverse-calculation)
    return calculateFinalDamage(baseDamage, distanceMod, terrainMod, coverMod, armorMod, avgRandom, criticalMod, config);
}
```
